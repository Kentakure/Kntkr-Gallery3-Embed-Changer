...
if(preg_match('{'.$sitepattern.'}',$the_content)){
	...
	//IMGタグの整形。大きさをリサイズに統一。Class属性等が記述されている場合は継承。
	$convert2 = preg_replace('{(<a.*>)?<img(.*)?src\=\"'.$sitepattern.'\/var\/(albums|resizes|thumbs)\/([-_.!~*\'()a-zA-Z0-9;\/:@&=+$,%#]+)\.([^?"<>]*)(?:\?[^"<>]*)?\"([^<>]*)?>(<\/a>)?}', '<div class="kntkrg3ec" style="text-align:center;">$1<img$2src="$3://'.$option_siteindex.'/var/resizes/$5.$6"$7>$8</div>', $convert1);
	...
	$convert4 = preg_replace('{<img(.*)?src\=\"'.$sitepattern.'\/var\/resizes\/([-_.!~*\'()a-zA-Z0-9;\/:@&=+$,%#]+)(?:\?[^"<>]*)?\"([^<>]*)?>}','<img$1src="'.$thumbs.'$3"$4>',$convert3);
	//代用したサムネイル画像のリンク先を新規ウィンドウで開くに設定変更。
	$convert5 = preg_replace('{<a(.*)?href\=\"'.$sitepattern.'\/var\/resizes\/([-_.!~*\'()a-zA-Z0-9;\/:@&=+$,%#]+)\"([^<>]*)?><img(.*)?><\/a>}','<a$1href="'.$resizes.'$3"$4target="_blank"><img$5></a>',$convert4);
	$the_content = $convert5;
	}else{
	//PC表示の場合にはリサイズのままで申し送る。
	$the_content = $convert3;
	}
}
return $the_content;
}
